...
# --- Custom CSS for UI Enhancements (Bigger Sidebar & Print-Safe fixes) ---
st.markdown("""
    <style>
        ...
        /* Increase overall text sizes in the sidebar for better showcasing */
        ...
        /* Increase standard widget labels (Selectbox, File Uploader) */
        [data-testid="stSidebar"] label p {
            font-size: 1.25rem !important;
        }
        /* Increase info/alert box text */
        [data-testid="stSidebar"] [data-testid="stAlert"] div div p {
            font-size: 1.2rem !important;
        }
        /* Make DataFrame containers globally consistent */
        [data-testid="stDataFrame"] {
            margin-top: 10px !important;
            margin-bottom: 20px !important;
        }
        
        /* --- DEDICATED PRINT/PDF CSS RULES --- */
        @media print {
            /* Force DataFrame grids to not float over preceding elements */
            [data-testid="stDataFrame"] {
                position: static !important;
                margin-top: 20px !important;
                page-break-inside: avoid !important;
            }
            /* Ensure custom HTML alerts are respected as standard blocks */
            .print-safe-alert {
...
with st.sidebar:
    ...
    # Dynamically assign the API key
    ...
    if dataset_choice == "Custom Upload (CSV/Excel)":
        ...
        **Data Guidelines:**
        - **Formats:** `.csv` or `.xlsx`
        - **Limits:** Max {config.MAX_ROWS_UPLOAD} rows for demo batching limits.
        """)
        ...
        if uploaded_file is not None:
            try:
                ...
                # CLEANUP: Remove pandas 'Unnamed' index columns
                ...
                st.error("Encoding Error: Please save your CSV file as UTF-8.")
                st.stop()
...
if selected_db_path and os.path.exists(selected_db_path):
    ...
    st.error("Datasets not found! Please generate datasets first.")
    st.stop()
...
with tab_app:
    ...
    st.write("") # Small vertical whitespace between the two tables
    ...
    if st.session_state.gold_ready:
        ...
        # Dedicated container to ensure chat inputs stay pinned at the bottom
...
with tab_architecture:
    ...
    st.markdown("""
    This project adheres to the **Principle of Least Privilege** and uses a simulated **Medallion Data Architecture**.
    """)

    # Visual representations of the architecture flow
    col_a, col_b, col_c = st.columns(3)

    with col_a:
        st.error("### 🔴 Phase 1: Ingestion\n**Bronze Database**")
        st.write("Raw data containing sensitive Personally Identifiable Information (PII) is ingested from internal enterprise systems (CSV/SQL).")
